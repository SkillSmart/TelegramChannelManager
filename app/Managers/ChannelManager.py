...
import os.path
import time
from app.Connectors import ClientConnector
...
class ChannelManager:
    ...
    *   Collect information about the registered Users in our channel for model training
    •	Provide a convenient interface to connect our user accounts to the Telegram API to deploy the script in
        parallel to make the best use of our time before getting ultimately throttled by Telegram
    ...
    def __init__(self, channel=None, client=None):
        if not client:
            self.client = ClientConnector.connect_client()
    ...
    def retrieve_participants(self, limit=100000, offset=0, force_refetch=False):
        ...
        if os.path.exists('bin/Participant-list{}-{}.pkl'.format(self.channel.title, limit)) and not force_refetch:
            with open('bin/Participant-list{}-{}.pkl'.format(self.channel.title, limit), 'rb') as input:
                self.channel_participants = pickle.load(input)
        else:
            # Cache the Participants to the object
            print("Indexing channel participants for Channel '{}'....".format(self.channel.title))
            self.channel_participants = self.client.get_participants(self.channel, aggressive=aggressive)

            if not os.path.exists('bin'):
                os.mkdir('bin')
            with open('bin/Participant-list{}-{}.pkl'.format(self.channel.title, limit), 'wb') as output:
    ...
    def create_userdict(self, force_refetch=False):
        ...
        if not self.channel_participants:
            self.retrieve_participants(force_refetch=force_refetch)
    ...
    def restrict_users(
            self, userlist, type=['bann', 'read-only', 'write-only', 'no-pictures'],
            offset= None,  limit=None, calls_throttle=20, force_refetch=False
        ):
        ...
        for id, userid in enumerate(userlist):
            ...
            try:
                ...
                throttle = 60 / calls_throttle + 3.8
                time.sleep(throttle)
                n_banned += 1
            except FloodWaitError as e:
                print("Sleeping for overflow throttle for  {} seconds".format(e.seconds))
